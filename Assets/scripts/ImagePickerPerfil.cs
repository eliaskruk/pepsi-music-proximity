...
public class ImagePickerPerfil : MonoBehaviour {
	...
	private void test_guardar(){
		...
		imageTexture.GetComponent<Renderer>().material.mainTexture = tex;

		GMS.userData.temp_img = GMS.generateId ().ToString () + ".png";
		
		StartCoroutine (GMS.saveTextureToFile (imageTexture.GetComponent<Renderer>().material.mainTexture as Texture2D, GMS.userData.temp_img, 'u'));
	...
	private void PickImageFinished (ePickImageFinishReason _reason, Texture2D _image)
	{
		...
		if (reasonString == "SELECTED") {
			...
			image = _image;
			imageTexture.GetComponent<Renderer>().material.mainTexture = image;
